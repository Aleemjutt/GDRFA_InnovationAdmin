...
import { VersionModel } from 'src/app/_models/knowledge/version';
...
export class InnovationversionsComponent implements OnInit {
  ...
  constructor(
    ...
    private versionService: InnovationVersionsService,
  ...
  initilizeDataTable(): void {
    ...
    this.versionService._list().subscribe((response: ResponseResult) => {
      ...
      this.versionModelList = response.data;
      // Datatable reloading
      datatable.destroy();
  ...
  addWithFile() {
    this.versionService._add(this.versionModel).subscribe({
  ...
  _update() {
    this.versionService._update(this.versionModel).subscribe({
      next: (response: ResponseResult) => {
  ...
  viewDetails(id: number, templateDetails: TemplateRef<any> | undefined) {
    this.objeInit();
    this.versionService._details(id).subscribe({
      next: (response: ResponseResult) => {
        if (response.statusCode == StatusCodes.success || StatusCodes.update) {
          if (templateDetails) {
  ...
  delete(id: number) {
    this.versionService._delete(id).subscribe({
      next: (response: ResponseResult) => {
              ...
              if (uploadedImages === totalImages) {
                for (var _attachment of this.attachmentList) {
                  if (_attachment.instanceName == 'bookfile') {
                    this.versionModel.bookUrl = _attachment.url;
                  } else if (_attachment.instanceName == 'bookCoverImage') {
                    this.versionModel.url = _attachment.url;
                  }
                }
  ...
  removeImage(instanceName: string) {
    if (this.versionModel) {
      if (instanceName == 'bookCoverImage') {
        this.versionModel.url = '';
        this.versionModel.urlBase64 = '';
      } else {
        this.versionModel.bookUrl = '';
        this.versionModel.bookUrlBase64 = '';
      }
    }
  }
}
