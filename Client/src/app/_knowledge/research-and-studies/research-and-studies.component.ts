...
import { ResearchAndStudiesModel } from 'src/app/_models/knowledge/researchAndStudies';
...
export class ResearchAndStudiesComponent implements OnInit {
  ...
  researchAndStudiesModel: ResearchAndStudiesModel;
  researchAndStudiesModelList: any;
  modalRef?: BsModalRef;
  fb: any;
  ...
  constructor(
    ...
    private researchAndStudyService: ResearchAndStudiesService,
    public globalService: GlobalServiceService
  ) {
    this.researchAndStudiesModel = {
      id: 0,
      discuss: null, // Set a default value for discuss from the Discuss enum
      researchAndStudiesCategories: null, // Provide appropriate default value or structure for ResearchAndStudiesCategories
      descrtionEn: '',
      descrtionAr: '',
      publisherNameEn: '',
      publisherNameAr: '',
  ...
  pondHandleAddFile(event: any, instanceName: string) {
    ...
    if (instanceName === 'ResearchAndStudy') {
      this.file = event.file.file;
    } else {
      this.attachmentFile = event.file.file;
    }
  ...
  initilizeDataTable(): void {
    ...
    this.researchAndStudyService
      ...
      .subscribe((response: ResponseResult) => {
        ...
        this.researchAndStudiesModelList = response.data;
        // Datatable reloading
        datatable.destroy();

        setTimeout(() => {
          $('#researchAndStudiesDataTable').DataTable({
            ...
            columns: [
              { data: 'id' },
              {
                data: 'descrtionEn', //(row: any) => this.getDepartmentName(row.requestModel.sID),
              },
              {
                data: 'descrtionAr', //(row: any) => this.getDepartmentName(row.requestModel.sID),
              },
              {
                data: 'storyNameEn', //(row: any) => this.getDepartmentName(row.requestModel.sID),
              },
              {
                data: 'storyNameAr', //(row: any) => this.getDepartmentName(row.requestModel.sID),
              },
              {
                data: 'publisherNameEn', //(row: any) => this.getDepartmentName(row.requestModel.sID),
              },
              {
                data: 'publisherNameAr', //(row: any) => this.getDepartmentName(row.requestModel.sID),
              },
  ...
  addWithFile() {
    this.researchAndStudyService._add(this.researchAndStudiesModel).subscribe({
  ...
  _update() {
    this.researchAndStudyService
      ._update(this.researchAndStudiesModel)
      .subscribe({
        next: (response: ResponseResult) => {
          if (
            response.statusCode == StatusCodes.success ||
  ...
  viewDetails(id: number, templateDetails: TemplateRef<any> | undefined) {
    this.objeInit();
    this.researchAndStudyService._details(id).subscribe({
      next: (response: ResponseResult) => {
        if (response.statusCode == StatusCodes.success || StatusCodes.update) {
          ...
          this.researchAndStudiesModel = response.data;
        } else {
          this.tosterService.error(response.message);
        }
      },
  ...
  delete(id: number) {
    this.researchAndStudyService._delete(id).subscribe({
      next: (response: ResponseResult) => {
  ...
  upload(file: File | undefined, type: number): void {
    ...
    if (file) {
      ...
      this.uploadService
        .upload(this.currentFile, 'ResearchAndStudy')
        .pipe(
          finalize(() => {
            if (this.attachmentList.length > 0) {
              this.researchAndStudiesModel.url =
  ...
  removeImage() {
    if (this.researchAndStudiesModel) {
      this.researchAndStudiesModel.url = '';
      this.researchAndStudiesModel.urlBase64 = '';
    }
  }
}
