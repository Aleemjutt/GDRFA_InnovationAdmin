      ...
      next: (response: ResponseResult) => {
        if (response.statusCode == 0) {
          if (template) {
            this.partnerModel = response.data;
            if (
              this.partnerModel.imageUrlView != null &&
              this.partnerModel.imageUrlView != ''
            )
              this.pondFiles = [
                {
                  source: this.partnerModel.imageUrlView,
                  options: {
                    type: 'local',
                  },
                },
        ...
        setTimeout(() => {
          $('#partnersDataTable').DataTable({
            ...
            columns: [
              ...
              //   data: 'id',
              //   render: (id: any) => `
              //     <a  routerLink="/partner/${id}/view" class="btn btn-info">View</a>
              //     <a href="#" [routerLink]="['/partner/', ${id}, 'edit']" class="btn btn-primary">Edit</a>
