...
import { CreditService } from 'src/app/_services/_about/credit.service';
...
export class CreditsListComponent implements OnInit {
  ...
  // creditModel: CreditModel | undefined = undefined;
  ...
  initilizeDataTable(): void {
    this.creditService.getCreditList().subscribe((response: ResponseResult) => {
      ...
      // ////console.log(this.creditList, 'List Data');

      setTimeout(() => {
        $('#creditsDataTable').DataTable({
          ...
          rowCallback: (row: Node, data: any, index: number) => {
            const viewCreditDetails = $('button:first', row);
            const editCredit = $('button:eq(1)', row);
            const deleteCredit = $('button:last', row);
            // Attach click event handlers to the buttons
            viewCreditDetails.on('click', () => {
              this.viewCreditDetails(data.id, this.templateDetails);
            });

            editCredit.on('click', () => {
              this.editCreditDetails(data.id, this.template);
            });

            deleteCredit.on('click', () => {
              this.deleteCredit(data.id);
            });
  ...
  addWithFile() {
    this.creditService.addCredit(this.creditModel).subscribe({
  ...
  updateCredit() {
    this.creditService.updateCredit(this.creditModel).subscribe({
      next: (response: ResponseResult) => {
        if (response.statusCode == 0) {
          this.modalRef?.hide();
          this.tosterService.success(response.message);
  ...
  viewCreditDetails(id: number, templateDetails: TemplateRef<any> | undefined) {
    this.creditService.getCreditDetails(id).subscribe({
      next: (response: ResponseResult) => {
        if (response.statusCode == 0) {
          if (templateDetails) {
          ...
          this.creditModel = response.data;
        } else {
          this.tosterService.error(response.message);
        }
      },
  ...
  editCreditDetails(id: number, template: TemplateRef<any> | undefined) {
    this.creditService.getCreditDetails(id).subscribe({
      next: (response: ResponseResult) => {
        if (response.statusCode == 0) {
          if (template) {
            this.creditModel = response.data;
            if (
              this.creditModel.imageUrlView != null &&
              this.creditModel.imageUrlView != ''
            )
              // this.pondFiles = [
              //   {
              //     source: this.creditModel.imageUrlView,
              //     options: {
              //       type: 'local',
              //     },
              //   },
      ...
      setTimeout(() => {
        $('#creditsDataTable').DataTable({
          ...
          columns: [
            ...
            data-bs-toggle="modal"
            data-bs-target="#exampleModal"
            (click)="editCreditDetails(id, template)"
            data-backdrop="static"
            data-keyboard="false"
          >
            <svg
  ...
  upload(file: File | undefined, type: number): void {
    ...
    if (file) {
      ...
      this.uploadService
        .upload(this.currentFile, 'Credit')
        .pipe(
          finalize(() => {
            if (this.attachmentList.length > 0) {
              this.creditModel.imageUrl = this.attachmentList[0].imageUrl;
              if (type == 0) {
                this.addWithFile();
              } else {
                this.updateCredit();
